...
**When to Use**:
- Implementing new stream consumers
- Adding Bybit/CMC data sources
...
**Example Prompts**:
```
Add a new Bybit linear stream consumer for SOL/USDT

Implement retry logic in AmqpPublisher for failed publishes

Fix the WebSocket reconnection logic in BybitSpotConsumer

Add support for new CMC endpoints in CmcParserModule
```
